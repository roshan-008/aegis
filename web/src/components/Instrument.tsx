...
const TIMEFRAMES: [string, string][] = [
  ["1M", "1mo"], ["6M", "6mo"], ["1Y", "1y"], ["5Y", "5y"], ["10Y", "10y"], ["Max", "max"],
];
const rangeLabel = (r: string) =>
  (TIMEFRAMES.find(([, v]) => v === r)?.[0] || r).toLowerCase();
...
export default function Instrument({
  data, stats, quote, range, onRange, rate, onRate, onBack,
}: {
  data: Analysis; stats: Stats; quote: Quote | null;
    ...
    <>
      ...
      <div className="ihead">
        ...
        <div className="price">
          ...
          <span className={"d " + (ret >= 0 ? "up" : "down")}>{pct(ret)} · {lbl}</span>
        </div>
        <div className="feed">
          {quote && (
          ...
          <span className="chip">{data.rows} bars · {data.parallel ? data.threads + " threads" : "1 core"}</span>
        </div>
      </div>

      <div className="controls">
        <div className="tf">
          {TIMEFRAMES.map(([label, val]) => (
            <button key={val} className={range === val ? "on" : ""} onClick={() => onRange(val)}>{label}</button>
          ))}
        </div>
        <label className="rate">
          <span>tape</span>
          <input
            type="number" min={0} max={30} step={1} value={rate}
            onChange={(e) => onRate(Math.max(0, Math.min(30, Number(e.target.value) || 0)))}
          />
          <span>ticks/s</span>
        </label>
      </div>
      ...
      <div className="tiles">
        <Tile k="Total return" v={pct(ret)} s={lbl} cls={ret >= 0 ? "up" : "down"} />
        ...
        <Tile k="Range" v={px(stats.min) + " – " + px(stats.max)} s={lbl + " low / high"} />
      </div>

      <div className="grid split">
        <div className="card">
          <h3>Price, rolling mean &amp; VWAP</h3>
          <div className="sub">Scroll to zoom, drag to pan. {data.window}-period rolling mean and VWAP; volume histogram flags block days.</div>
        ...
        <div className="card">
          ...
          <table className="book">
            <tbody>
              ...
              <tr><td>Notional traded ({lbl})</td><td>{symbol}{big(stats.dollar_volume)}</td></tr>
